...
import type { NewsListPayload } from "@/lib/news/get-public-news-list";
...
export function NewsPageClient({ initialData }: { initialData: NewsListPayload }) {
  const [page, setPage] = React.useState(initialData.pagination.page);
  const [articles, setArticles] = React.useState(initialData.items);
  const [totalPages, setTotalPages] = React.useState(initialData.pagination.totalPages);
  const [totalCount, setTotalCount] = React.useState(initialData.pagination.total);
  ...
  /** Server already loaded default view; skip refetch until filters/pagination change. */
