...
/** Admin-only: upload an image; returns a public URL for article body or featured image. */
export async function POST(request: Request): Promise<NextResponse<UploadResponse>> {
  ...
  try {
    ...
    return errorResponse(
      500,
      "BUCKET_INIT_FAILED",
      "Could not create or verify the news-images storage bucket.",
  ...
  const path = `uploads/${Date.now()}-${randomName()}.${safeExt}`;
